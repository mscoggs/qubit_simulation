...
void mcdb_simulation(Simulation_Parameters& sim_params){
	...
	double *j_array, *k_array,*b_array,*j_temp, *k_temp, *b_temp,*temp_saved_states, old_mc_result, new_mc_result, change;
	...
	for (i=0;i<TEMP_DECAY_ITERATIONS+ZERO_TEMP_ITERATIONS;i++){
		...
		for (j=0; j<sim_params.total_sweeps*sim_params.sweeps_multiplier;j++){
			...
			if (new_mc_result<sim_params.best_mc_result) sim_params.best_mc_result=new_mc_result, old_mc_result=new_mc_result,  copy_arrays_mcdb(sim_params, sim_params.j_best, sim_params.k_best, sim_params.b_best,j_array, k_array, b_array, 0, 0), std::memcpy(sim_params.best_evolved_state,sim_params.state, 2*sim_params.N*sizeof(double));
